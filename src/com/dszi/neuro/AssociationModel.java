...
public class AssociationModel {
    ...
    {
        if(arg1<0 || arg1>99) System.out.println("ERROR! Index out of bounds!");
        else
        {
            if (arg2 != -1 && arg2 != 1) System.out.println("ERROR! Attempted to alter the value to something that wasn't 1 or -1!");
            else data[arg1] = arg2;
        }
    }
    ...
    {
        if (arguments.length != 100) System.out.println("ERROR! Attempted to input a table that didn't have 25 elements!");
        else
        {
            boolean flag = true;
            for (int i = 0; i < 25; i++) if (arguments[i] != 1 && arguments[i] != -1) { System.out.println("ERROR! Value of arguments[" + i + "] is not 1 or -1!"); flag = false; }
            if(flag==true) for (int i = 0; i < 25; i++) Alter(i, arguments[i]);
        }
    }
    ...
    public double[] AssocMod(double[] idealComponent, double[] proposedComponent)
    {
        ...
        for (int i = 0; i < 201; i++)
        {
            for (int j = 0; j < 201; j++)
            {
                if (j == 200) { tempold = tempnew; tempnew = W[i][j] * Z0.Get(j); tempnew = tempold + tempnew; resultz0[i] = tempnew; tempnew = 0; tempold = 0; }
        ...
        for (int i = 0; i < 201; i++)
        {
            for (int j = 0; j < 201; j++)
            {
                if (j == 200) { tempold = tempnew; tempnew = W[i][j] * Z1.Get(j); tempnew = tempold + tempnew; resultz1[i] = tempnew; tempnew = 0; tempold = 0; }
        ...
        for (int i = 0; i < 201; i++)
        {
            for (int j = 0; j < 201; j++)
            {
                if(resultz0[i]!=idealComponent[i]) continues = true;  
            }
        }
        }
        continues=false;
