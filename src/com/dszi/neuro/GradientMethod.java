...
public class GradientMethod {
	public class Polynomial {
		//Czterowymiarowa funkcja (wielomian 4tego stopnia) wagi, okreœlona dla ka¿dego elementu gridu.
		//Argumenty:
		...
		public double get_coeff(char which, int at)
        {
            ...
            if(which == 'D' || which == 'd') result = coeffD[at];
            if(which == 'N' || which == 'n') result = coeffN[at];
            if(which == 'Z' || which == 'z') result = coeffZ[at];
            if(which == 'I' || which == 'i') result = coeffI[at];
            if (which == 'F' || which == 'f') result = freecoeff;
            if (result == Double.MAX_VALUE) System.out.println("--GM--Getcoeff--FAILED"); 
            return result;
        }
		...
		public double get_value(double[] values) //ma strukturê (D,N,Z,I) - lokalna dla ka¿dego pola.
        {
            ...
            for(int j = 0; j<4; j++)
            {
                if (j == 0) for (int i = 0; i < 5; i++) result = result - get_coeff('D', i) * Math.pow(values[j], (5 - i));
                if (j == 1) for (int i = 0; i < 5; i++) result = result - get_coeff('N', i) * Math.pow(values[j], (5 - i));
                if (j == 2) for (int i = 0; i < 5; i++) result = result + get_coeff('Z', i) * Math.pow(values[j], (5 - i));
                if (j == 3) for (int i = 0; i < 5; i++) result = result + get_coeff('I', i) * Math.pow(values[j], (5 - i));
            }
		...
		public Point GradientMethodOutput(int tractor_x, int tractor_y, SingleCell CellPanel[][], boolean[][] visited)
		{
			...
			for(int i = 0; i<Constants.gridSizeX; i++){
				for(int j = 0; j<Constants.gridSizeY; j++){
					...
					value[x][y] = F.get_value(new double[] 
							{d_at(tractor_x,tractor_y,x,y),
