...
public class NeuroAlgorithm {
	...
	public NeuroAlgorithm(SingleCell[][] cellPanel) {
		...
		for(int i=0; i<Constants.gridSizeX; i++) {
			for(int j=0; j<Constants.gridSizeY; j++) {
				if(cellPanel[i][j].getIrrigation()!=100 ||
				   cellPanel[i][j].getSoilDestruction()!=0 ||
				   cellPanel[i][j].getNumberOfPests()!=0) stop_flag=false;
			}
		}
		
		if(stop_flag==true) System.out.println("Algorytm zakoñczy³ dzia³anie - wszystkie pola w porz¹dku!");
		else {
			this.cellPanel = cellPanel;
	
			initializeTractor();
			if(isNotOutOfBounds(positionX,positionY)==true) logic(0,0,cellPanel);
			else System.out.println("--NA--isNotOutOfBounds--values out of bound.");
	...
	public void logic(int positionX, int positionY, SingleCell[][] cellpanel) {
		...
		while(current_x!=target_x) {
			if(target_x>current_x) current_x++;
			else current_x--;
			continuewithoutCalculations(current_x, current_y);
		}
		
		while(current_y!=target_y) {
			if(target_y>current_y) current_y++;
			else current_y--;
			continuewithoutCalculations(current_x, current_y);
		}
